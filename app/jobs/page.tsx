...
export default function Jobs(){
  ...
  const [cat, setCat] = useState(state.categories.includes(paramCat) ? paramCat : 'All')
  ...
  useEffect(()=>{ const c = sp.get('cat'); if (c && state.categories.includes(c)) setCat(c) }, [sp, state.categories])
  ...
  const exportCSV = ()=>{
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'airlink-jobs.csv'; a.click(); URL.revokeObjectURL(url)
  }
  ...
  return (
    <section className="section">
      ...
      {!loading && filtered.length===0 && (
        <div className="card p-6 mt-3 text-center text-slate-600">No jobs match your filters.</div>
      )}
    </section>
  )
}
